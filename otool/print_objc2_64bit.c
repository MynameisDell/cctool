...
void (*func)(uint64_t, struct info *))
{
	...
	for(i = 0; i < s->size; i += sizeof(uint64_t)){
	    ...
	    printf("%016llx ", (unsigned long long)(s->addr + i));
...
struct info *info)
{
	...
	printf("           isa 0x%llx", (unsigned long long)c.isa);
	...
	if(name != NULL)
	    ...
	    printf("\n");
	printf("    superclass 0x%llx", (unsigned long long)c.superclass);
	...
	if(name != NULL)
	    ...
	    printf("\n");
	printf("         cache 0x%llx", (unsigned long long)c.cache);
	...
	if(name != NULL)
	    ...
	    printf("\n");
	printf("        vtable 0x%llx", (unsigned long long)c.vtable);
...
struct info *info)
{
	...
	printf("               ivarLayout 0x%llx\n", (unsigned long long)cro.ivarLayout);
	print_layout_map(cro.ivarLayout, info);
	printf("                     name 0x%llx", (unsigned long long)cro.name);
...
char *indent)
{
	...
	for(i = 0; i < ml.count; i++){
	    ...
	    printf("%s\t\t      name 0x%llx", indent, (unsigned long long)m.name);
		...
		printf("\n");
	    printf("%s\t\t     types 0x%llx", indent, (unsigned long long)m.types);
		...
		printf("\n");
	    printf("%s\t\t       imp 0x%llx", indent, (unsigned long long)m.imp);
...
struct info *info)
{
	...
	for(j = 0; j < il.count; j++){
	    ...
	    printf("\t\t\t     name 0x%llx", (unsigned long long)i.name);
		...
		printf("\n");
	    printf("\t\t\t     type 0x%llx", (unsigned long long)i.type);
...
struct info *info)
{
	...
	for(i = 0; i < pl.count; i++){
	    ...
	    printf("\t\t\t      isa 0x%llx\n", (unsigned long long)pc.isa);
	    printf("\t\t\t     name 0x%llx", (unsigned long long)pc.name);
...
struct info *info)
{
	...
	for(j = 0; j < opl.count; j++){
	    ...
	    printf("\t\t\t     name 0x%llx", (unsigned long long)op.name);
		...
		printf("\n");
	    printf("\t\t\tattributes x%llx", (unsigned long long)op.attributes);
...
struct info *info)
{
	...
	printf("              name 0x%llx", (unsigned long long)c.name);
...
struct info *info)
{
	...
	for(i = 0; i < s->size; i += sizeof(struct message_ref)){
	    ...
	    printf("  imp 0x%llx", (unsigned long long)mr.imp);
		...
		printf("\n");
	    printf("  sel 0x%llx", (unsigned long long)mr.sel);
...
enum bool verbose)
{
	...
	for(s = string_objects;
	    ...
	    printf("String Object 0x%llx\n",
		   (unsigned long long)(string_objects_addr + ((char *)s - (char *)string_objects)));

	    if(info.swapped)
		swap_string_object_64(&string_object, info.host_byte_sex);
	    printf("           isa 0x%llx", (unsigned long long)string_object.isa);
		...
		printf("\n");
	    printf("    characters 0x%llx", (unsigned long long)string_object.characters);
	    if(verbose){
