...
struct ofile *ofile)
{
	...
	if(ofile->mh != NULL || ofile->mh64 != NULL)
	    size = stuff_round(ofile->object_size, 8);
#ifdef LTO_SUPPORT
	else if(ofile->lto != NULL && ofile->file_type == OFILE_LLVM_BITCODE)
	    size = stuff_round(ofile->file_size, 8);
#endif /* LTO_SUPPORT */
	else
	    size = stuff_round(ofile->member_size, 8);
	...
	if(ofile->member_ar_hdr == NULL){
		...
		ar_name_size = stuff_round(member->input_base_name_size, 8) +
			       (stuff_round(sizeof(struct ar_hdr), 8) -
		...
		if(ofile->member_name != ofile->member_ar_hdr->ar_name){
		    ...
		    ar_name_size = stuff_round(ar_name_size, 8) +
				   (stuff_round(sizeof(struct ar_hdr), 8) -
				    sizeof(struct ar_hdr));
		    sprintf(ar_name_buf, "%s%-*lu", AR_EFMT1,
			    (int)(sizeof(member->ar_hdr.ar_name) -
		     ...
		     */
		    member->member_name_size = size_ar_name(&member->ar_hdr);
		    ar_name_size = stuff_round(ofile->member_name_size, 8) +
				   (stuff_round(sizeof(struct ar_hdr), 8) -
				    sizeof(struct ar_hdr));
		    member->output_long_name = TRUE;
		    sprintf(ar_name_buf, "%s%-*lu", AR_EFMT1,
...
char *output)
{
	...
	for(i = 0; i < narchs; i++){
		...
		if(arch->members[j].output_long_name == TRUE){
		    ...
		    p += stuff_round(arch->members[j].member_name_size, 8) +
			       (stuff_round(sizeof(struct ar_hdr), 8) -
				sizeof(struct ar_hdr));
		}
		...
		pad = stuff_round(arch->members[j].object_size, 8) -
		      arch->members[j].object_size;
...
{ 
	...
	if(before != NULL && before->offset + before->size == offset){
		...
		if(after->written_size == 0)
		    write_size = stuff_trunc(after->offset + after->size -
				       write_offset, host_pagesize);
		else
		    write_size = stuff_trunc(after->written_offset - write_offset,
				       host_pagesize);
		if(write_size != 0){
		 ...
		 */
		write_offset = before->written_offset + before->written_size;
		write_size = stuff_trunc(offset + size - write_offset, host_pagesize);
		if(write_size != 0)
		    before->written_size += write_size;
		before->size += size;
	    }
	 ...
	 */
	     ...
	     * new area.
	     */
	    write_offset = stuff_round(offset, host_pagesize);
	    if(after->written_size == 0)
		write_size = stuff_trunc(after->offset + after->size - write_offset,
				   host_pagesize);
	    else
		write_size = stuff_trunc(after->written_offset - write_offset,
				   host_pagesize);
...
/*
 * stuff_trunc() truncates the value 'v' to the power of two value 'r'.  If v is
...
char *output)
{
	...
	arch->toc_strsize = stuff_round(arch->toc_strsize, 8);
	...
	if(cmd_flags.s == TRUE){
		...
		if(cmd_flags.use_long_names == TRUE){
		     ...
		     * (stuff_round(sizeof(struct ar_hdr), 8) - sizeof(struct ar_hdr)
		     * is 4 bytes.
		     */
		    ar_name = AR_EFMT1 "20";
		}
