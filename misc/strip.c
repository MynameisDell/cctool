...
#else
    enum bool unix_standard_mode __attribute__((unused));
#endif
    int cwd_fd;
    char *rename_file;
#ifndef NMEDIT
...
enum bool all_archs)
{
	...
	for(i = 0; i < narchs; i++){
		...
		for(j = 0; j < archs[i].nmembers; j++){
			...
			size = stuff_round(archs[i].members[j].member_name_size, 8) +
			...
			size += 
			   stuff_round(archs[i].members[j].object->object_size -
...
struct object *object)
{
	...
	if(sfile != NULL || Rfile != NULL || dfile != NULL || Aflag || uflag ||
	   ...
	   object->mh_filetype == MH_DYLINKER)
		...
		if(object->code_sig_cmd != NULL){
		    object->input_sym_info_size =
			stuff_round(object->input_sym_info_size, 16);
		    object->input_sym_info_size +=
			object->code_sig_cmd->datasize;
#ifndef NMEDIT
		    if(cflag){
		    ...
		    {
			object->output_sym_info_size =
			    stuff_round(object->output_sym_info_size, 16);
			object->output_sym_info_size +=
			    object->code_sig_cmd->datasize;
		    }
		}
...
enum byte_sex host_byte_sex)
{
    ...
    uint64_t n_value;
#else
    uint64_t n_value __attribute__((unused));
#endif
#ifdef NMEDIT
...
enum byte_sex host_byte_sex)
{
    ...
    uint64_t value64;
#ifdef NMEDIT
    uint32_t n_strx __attribute__((unused));
    enum bool made_local __attribute__((unused));
#else
    uint32_t n_strx;
...
unsigned long nextrefsyms)
{
	...
	new_strsize = stuff_round(new_strsize, sizeof(long));
...
#ifdef NMEDIT
    enum byte_sex host_byte_sex __attribute__((unused));
    long missing_reloc_symbols __attribute__((unused));
    enum bool edit_symtab_return __attribute__((unused));
#else
    enum byte_sex host_byte_sex;
    long missing_reloc_symbols;
