...
unsigned long narchs)
{
	...
	for(i = 0; i < narchs; i++){
		...
		for(j = 0; j < archs[i].nmembers; j++){
			...
			size = stuff_round(archs[i].members[j].member_name_size,
				     sizeof(long));
...
struct object *object)
{
	...
	if(object->code_sig_cmd != NULL){
	    object->input_sym_info_size = stuff_round(object->input_sym_info_size,
						16);
	...
	if(object->code_sig_cmd != NULL){
		...
		if(object->seg_linkedit->filesize >
		   object->seg_linkedit->vmsize)
		    object->seg_linkedit->vmsize =
			stuff_round(object->seg_linkedit->filesize,
		...
		if(object->seg_linkedit64->filesize >
		   object->seg_linkedit64->vmsize)
		    object->seg_linkedit64->vmsize =
			stuff_round(object->seg_linkedit64->filesize,
			      get_segalign_from_flag(&arch_signs[i].arch_flag));
	    }
	    ...
	    object->output_sym_info_size = stuff_round(object->output_sym_info_size,
						 16);
	 ...
	 */
	    ...
	    object->code_sig_cmd->dataoff = stuff_round(linkedit_end, 16);
	    object->output_code_sig_data_size = arch_signs[i].datasize;
	    object->output_code_sig_data = NULL;
	    ...
	    if(object->seg_linkedit != NULL){
		object->seg_linkedit->filesize =
		    stuff_round(object->seg_linkedit->filesize, 16) +
		    object->code_sig_cmd->datasize;
		if(object->seg_linkedit->filesize >
		   object->seg_linkedit->vmsize)
		    object->seg_linkedit->vmsize =
		...
		object->seg_linkedit64->filesize =
		    stuff_round(object->seg_linkedit64->filesize, 16) +
		    object->code_sig_cmd->datasize;
		if(object->seg_linkedit64->filesize >
		   object->seg_linkedit64->vmsize)
		    object->seg_linkedit64->vmsize =
