	...
	for(i = 0; i < nsegs; i++){
		...
		if(segs[i].sgp->flags & SG_NORELOC){
			...
			while(rp != NULL){
				...
				sects[k + j].rp = rp;
				segs[i].modified = 1;
				sp->size = stuff_round(rp->size, 1 << sp->align);
				break;
			    }
			    rp = rp->next;
			}
		    ...
		    if(strcmp(segs[i].sgp->segname, SEG_LINKEDIT) != 0 ||
		       i != nsegs - 1){
			if(segs[i].sgp->filesize != stuff_round(oldsectsize,
							  pagesize))
			    fatal("contents of input file: %s not in a format "
				  "that the specified sections can be replaced "
				  "by this program", input);
			segs[i].sgp->filesize = stuff_round(newsectsize, pagesize);
			segs[i].sgp->vmsize = stuff_round(newsectsize, pagesize);
