...
void)
{
	...
	for(p = file_parts; p != NULL; p = p->next){
	    ...
	    printf("    offset = %llu\n", (long long unsigned int)p->offset);
	    printf("    size = %llu\n", (long long unsigned int)p->size);
	    if(prev != NULL)
		if(prev != p->prev)
	...
	for(fp = file_parts; fp != NULL; fp = fp->next){
		...
		if(sections == TRUE || sections64 == TRUE){
		    printf("Symbols on file page %lu virtual address 0x%llx to "
			   "0x%llx\n", page_number, (long long unsigned int)low_addr, (long long unsigned int)high_addr);
		    if(sections == TRUE)
...
uint64_t high_addr)
{
	...
	for(i = 0; i < count; i++){
	  printf("  0x%016llx %s\n", (long long unsigned int)sorted_symbols64[i].n_value,
