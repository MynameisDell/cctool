...
unsigned long *header_size)
{
	...
	for(i = 0; i < ncmds; i++){
		...
		if(id != NULL){
		    new_size = sizeof(struct dylib_command) +
			       stuff_round(strlen(id) + 1, cmd_round);
		...
		for(j = 0; j < nchanges; j++){
			...
			new_size = sizeof(struct dylib_command) +
				   stuff_round(strlen(changes[j].new) + 1,
					 cmd_round);
		...
		for(j = 0; j < nchanges; j++){
			...
			linked_modules_size = pbdylib1->cmdsize - (
				...
				stuff_round(strlen(dylib_name1) + 1, cmd_round));
	...
	for(i = 0; i < ncmds; i++){
		...
		for(j = 0; j < nchanges; j++){
			...
			pbdylib2->cmdsize =
			    sizeof(struct prebound_dylib_command) +
			    stuff_round(strlen(changes[j].new) + 1, cmd_round) +
			    linked_modules_size;
