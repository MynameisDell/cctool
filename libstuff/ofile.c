...
#endif /* __LITTLE_ENDIAN__ */
		if((int)check_fat_object_in_archive(ofile) == (int)FALSE)
		    goto fatcleanup;
	    }
...
struct ofile *ofile)
{
	...
	offset = ofile->member_offset + stuff_round(ofile->member_size,sizeof(short));
#ifdef OTOOL
...
struct ofile *ofile)
{
	...
	while(size > offset){
	    ...
	    offset += stuff_round(strtoul(ar_hdr->ar_size, NULL, 10),
			    sizeof(short));
	}
...
enum bool archives_with_fat_objects)
{
	...
	while(size > offset){
	    ...
	    offset += stuff_round(ofile->member_size, sizeof(short));
	}
	ofile->member_offset = 0;
	ofile->member_addr = NULL;
	ofile->member_size = 0;
