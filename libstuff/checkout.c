...
struct object *object)
{
	...
	if(object->mh64 != NULL &&
	   (object->dyst->nindirectsyms % 2) != 0){
	    rounded_offset = stuff_round(offset, 8);
	}
	else{
	...
	if(object->code_sig_cmd != NULL){
	    rounded_offset = stuff_round(rounded_offset, 16);
...
struct object *object)
{
	...
	if(object->st != NULL && object->st->nsyms != 0){
		 ...
		 */
		rounded_strend = stuff_round(strend, 8);
		...
		if(object->mh64 != NULL &&
		   (object->dyst->nindirectsyms % 2) != 0){
		    rounded_indirectend = stuff_round(indirectend, 8);
		}
		else{
