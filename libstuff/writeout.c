...
enum bool *seen_archive)
{
	...
	*seen_archive = FALSE;
	...
	for(i = 0; i < narchs; i++){
		...
		*seen_archive = TRUE;
		...
		if(archs[i].fat_arch != NULL)
		    file_size = stuff_round(file_size, 1 << archs[i].fat_arch->align);
	...
	if(narchs > 1 || archs[0].fat_arch != NULL){
		...
		offset = stuff_round(offset, 1 << archs[i].fat_arch->align);
	...
	for(i = 0; i < narchs; i++){
		...
		*seen_archive = TRUE;
		...
		for(j = 0; j < archs[i].nmembers; j++){
			...
			p += stuff_round(archs[i].members[j].member_name_size, 8) +
				   (stuff_round(sizeof(struct ar_hdr), 8) -
				    sizeof(struct ar_hdr));
		    }
