     ...
     char *line;
{
  ...
  if ((i.tm.opcode_modifier & (Jump | JumpByte | JumpDword))
      && i.op[0].disps->X_op == (int)O_constant)
    {
...
optimize_disp ()
{
  ...
  for (op = i.operands; --op >= 0;)
    if (i.types[op] & Disp)
      {
	if (i.op[op].disps->X_op == (int)O_constant)
	  {
...
output_interseg_jump ()
{
  ...
  *p++ = i.tm.base_opcode;
  if (i.op[1].imms->X_op == (int)O_constant)
    {
  ...
  if (i.op[0].imms->X_op != (int)O_constant)
  ...
  for (n = 0; n < i.operands; n++)
    {
      if (i.types[n] & Disp)
	{
	  if (i.op[n].disps->X_op == (int)O_constant)
	    {
	      int size;
	      offsetT val;
	  ...
	  else
	    {
	      ...
	      int sign = 0;
#else
	      int sign __attribute__((unused)) = 0;
#endif
  ...
  for (n = 0; n < i.operands; n++)
    {
      if (i.types[n] & Imm)
	{
	  if (i.op[n].imms->X_op == (int)O_constant)
	    {
	      int size;
	      offsetT val;
     ...
     char *imm_start;
{
  ...
  segT exp_seg = 0;
#else
  segT exp_seg __attribute__((unused)) = 0;
#endif
  expressionS *exp;
  ...
  if (exp->X_op == (int)O_absent || exp->X_op == (int)O_big)
    {
  ...
  else if (exp->X_op == (int)O_constant)
    {
